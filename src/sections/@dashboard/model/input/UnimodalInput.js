...
export default function UnimodalInput({ strategyName, setNodes, setEdges, showAlert }) {
  ...
  const [selectedInputDataset, setSelectedInputDataset] = useState('');
  ...
  useEffect(() => {
    setTimeout(() => get(
      'get-datasets-information', // Route
      (response) => setInputDatasets(response), // Response callback
  ...
  // Event handlers
  const handleSelectInputDataset = (event, newInputDataset) => {
    setSelectedInputDataset(newInputDataset.name);
  };
  ...
  const handleCreateModel = () => {
    ...
    const body = JSON.stringify({
      ...
      model_input: selectedInputDataset
    });
    setTimeout(() => post(
      body, // Body
      ...
      <FormControl fullWidth>
        <Grid item xs={ 12 } ms={ 12 } lg={ 12 } sx={ { border: 1, borderColor: theme.palette.divider, borderRadius: '10px', p: 3 } }>
          <Autocomplete
            disableClearable
            required
            id="input-dataset-selection"
            options={ inputDatasets }
            getOptionLabel={ (option) => option.name }
            onChange={ handleSelectInputDataset }
            sx={ { mb: 1 } }
            renderInput={ (params) => <TextField { ...params } label="Input Dataset" margin="normal" sx={ { mt: 0 } } /> }
          />
