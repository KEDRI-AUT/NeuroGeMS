...
export default function LateFusionInput({ strategyName, setNodes, setEdges, showAlert }) {
  ...
  useEffect(() => {
    setTimeout(() => get(
      `get-strategy-requirements${requestHeader({ strategy_name: strategyName })}`, // Route
      (response) => setRequiredInputs(response), // Response callback
  ...
  const handleUpdateModel = () => {
    ...
    const body = JSON.stringify({
      ...
      model_input: requiredInputs[inputTab].name
    });
    setTimeout(() => post(
      body, // Body
      ...
      <FormControl fullWidth>
        { requiredInputs.length === 0 ? (
          ...
          <>
            <Tabs
              value={ inputTab }
              onChange={ (event, newValue) => setInputTab(newValue) }
              variant="scrollable"
              scrollButtons="auto"
              sx={ { ml: 2 } }
            >
              { requiredInputs.map((input) => (
                <Tab key={ input.name } label={ input.name } />
              )) }
            </Tabs>
            <Grid item xs={ 12 } ms={ 12 } lg={ 12 } sx={ { border: 1, borderColor: theme.palette.divider, borderRadius: '10px', p: 3 } }>
              ...
              <Autocomplete
                ...
                options={ requiredInputs[inputTab].options }
                  ...
                  >
                    {modelParameterOptions[key].map((option) => (
                      <MenuItem key={ `select-option-${key}${option}` === null ? 'None' : option } value={ option === null ? 'None' : option } name={ key }>
                        { option === true ? 'True' : option === false ? 'False' : option === null ? 'None' : Object.prototype.toString.call(option) === '[object Array]' ? option.join(', ') : option }
                      </MenuItem>
                    ))}
                  </TextField>
                ))
              ...
              <Box sx={ { display: 'flex', justifyContent: 'flex-end', mt: 2 } }>
                <LoadingButton variant="contained" onClick={ handleUpdateModel } loading={ loading } loadingPosition="start" startIcon={ <Iconify icon="eva:checkmark-outline" /> }>
                  Save
                </LoadingButton>
              </Box>
            </Grid>
